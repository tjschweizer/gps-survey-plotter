...
always 0, no geoid model loaded), `Fix ID` (**4 = RTK fixed, 5 = float**; 0 =
tapped on the map, with an elevation of 0 that is not a height),
`Horizontal Accuracy`, `Vertical Accuracy`, `Speed` (m/s), `Bearing`, `PDOP`.

Outings are now recorded in a **field project** generated by Export ▸ SW Maps
field project (`gpsrtk/io/swmaps_field.py`) and exported back as `.swmz`,
which is preferred over the CSV zip. Its point layers:

- `plan` — outstanding plan shots for Stake Out; generated, dropped on import.
- `shots` — every reading, recorded as a new point (GNSS, or tapped under
  canopy): `station` (pick list: `P12`, a mark such as `BM1`, `TP1`),
  `rod (in)` (text, **inches**, as read: `63 3/8`), `setup` (letter), `type`
  (only for unplanned shots; a plan shot takes the plan's purpose).
- `control checks` — the fixed-height pole on a mark at the start and end of
  an outing: `mark`. The only source of check shots.

Mower tracks go in the same project. SW Maps' remarks box is the note.
Older exports used a spot layer with `height number` (rod, inches), `type`
and `base position` (a numbered setup); the readers still map those.
